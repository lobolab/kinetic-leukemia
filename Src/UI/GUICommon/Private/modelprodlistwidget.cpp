...
ModelProdListWidget::ModelProdListWidget(Model *m, QWidget * parent)
...
void ModelProdListWidget::addModelProd() {
}

void ModelProdListWidget::replaceModelProd() {
}
...
void ModelProdListWidget::removeSelectedModelProd() {
}
...
void ModelProdListWidget::updateList() {
  clear();
  createList();
}

void ModelProdListWidget::createList() {
  QList<int> labels = model_->calcProductLabels().toList();
  qSort(labels);
  QSet<int> labelsInUse = model_->calcProductLabelsInUse();
  ...
  for (int i = 0; i < nProducts; ++i) {
    ...
    //R QString str = preStr + QString("[%1] ini=%2 posLim=%3 negLim=%4 deg=%5")
    QString str = preStr + QString("[%1] ini=%2 lim=%3 deg=%4").arg(label)
      .arg(modelProd->init())
      .arg(modelProd->lim())
      .arg(modelProd->posLim())
      .arg(modelProd->negLim())
      .arg(modelProd->deg())
      .arg(modelProd->intrGrow());
