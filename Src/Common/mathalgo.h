...
inline double roundToHundredths(double n) {
  return floor(n * 100.0 + 0.5) / 100.0;
}

extern double t90[];

inline double marginOfErrorT90(double std, int n) {
  Q_ASSERT(n > 0 && n <= 30);
  return t90[n - 1 - 1] * std / sqrt(n);
}
...
inline double randNoise() {
  static std::normal_distribution<double> dist(0.0, 0.0001);
  return dist(rand_gen);
}

inline double randDouble(double min, double max) {
  return min + MathAlgo::rand01() * (max - min);
}
//
//inline double randDouble(double min, double max, double delta) {
//  return delta * round(randDouble(min, max) / delta);
//}
//
//inline double randDelta(double v, double min, double max,
//                        double delta) {
//  if (rand_gen() % 2)
//    v += delta;
//  else
